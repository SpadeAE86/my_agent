...
class IndexManager:
    def __init__(self):
        self.connector = elasticsearch_connector
    ...
    async def index_exists(self, index_name: str) -> bool:
        client = await self.get_client()
        try:
            return bool(await client.indices.exists(index=index_name))
        except Exception as e:
        ...
        try:
            ...
            for field_name in model_class.model_fields.keys():
                if field_name in field_types:
                    mappings["properties"][field_name] = field_types[field_name]
            
            # Use settings and mappings as kwargs directly for ES 8.x/9.x compatibility
            await client.indices.create(index=index_name, settings=resolved_settings, mappings=mappings)
            log.info(f"Created Elasticsearch index: {index_name}")
            return True
            
        except Exception as e:
            log.error(f"Failed to create Elasticsearch index {index_name}: {e}")
            raise
    ...
    async def delete_index(self, index_name: str) -> bool:
        ...
        try:
            ...
            log.info(f"Deleted Elasticsearch index: {index_name}")
            return True
        except Exception as e:
            log.error(f"Failed to delete Elasticsearch index {index_name}: {e}")
            raise
    
    async def update_index_settings(self, index_name: str, settings: Dict[str, Any]) -> bool:
        client = await self.get_client()
        try:
            await client.indices.put_settings(index=index_name, settings=settings)
    ...
    async def update_index_mapping(self, index_name: str, properties: Dict[str, Any]) -> bool:
        client = await self.get_client()
        try:
            await client.indices.put_mapping(index=index_name, properties=properties)
