...
CN_ANALYZER = "standard"  # change to "ik_max_word" after installing IK plugin in ES if needed


class CarInteriorAnalysisV2(BaseIndex):
    """
    Elasticsearch version of CarInteriorAnalysisV2 Index Model.
    - More structured filter fields (keyword/boolean)
    - Multiple dense_vector fields (formerly knn_vector)
    """
    ...
    end_time: Annotated[float, Float()] = Field(
        0.0, description="分镜结束时间（秒）。用于混镜回查/切片。"
    )
